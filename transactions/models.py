...
class PurchaseItem(models.Model):
    ...
    weight = models.CharField(
        max_length=6, choices=weight_CHOICES, default='kg')
    created_at = models.DateField(auto_now=True)
    totalprice = models.IntegerField(default=1)
    
    def __str__(self):
        return "Bill no: " + str(self.billno.billno) + ", Item = " + self.stock.name + str(self.created_at)
...
class SaleItem(models.Model):
    ...
    perprice = models.IntegerField(default=1)
    weight = models.CharField(
        max_length=6, choices=weights_CHOICES, default='kg')
...
class Expense(models.Model):
    ...
    def save(self, *args, **kwargs):
        self.total = int(self.salary) + int(self.kitchen) + int(self.maintenance) + int(
            self.internetbills)+int(self.gasbill)+int(self.waterbills)+int(self.electricitybill)
...
class SleepingPartners(models.Model):
    name = models.CharField(max_length=50)
    mobile_number = models.CharField(max_length=100)
    address = models.CharField(max_length=100)
...
class CreditBook(models.Model):
    supplier = models.ForeignKey(
        Supplier, on_delete=models.CASCADE, related_name='creditbooksupplier')
    total=models.IntegerField(default=0)
    paid=models.IntegerField(default=0)
    remaing=models.IntegerField(default=0)